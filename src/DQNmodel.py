...
class DeepQAgent:
    ...
    def train(self, batch_size, e):
        ...
        if e % inc_every_episode == 0 and e != 0:
    ...
    def run(self, timesteps=500, batch_size=32, average_size=100, explore=True, run=1):
        ...
        :param explore: boolean
        :param run: positive integer
        """
...
# agent = DeepQAgent(env, episodes)
# train = agent.run(episodes, max_score, batch_size, average_size, True, 1)
# test = agent.run(episodes, max_score, batch_size, average_size, False, 2)
#
